...
% getStrain.m
% Usage
%    getStrainMov(PMSpace,Protocol,time)
%
...
%  This version saves the result as a movie.
%
...
%Notes; for the movie, we cannot use a waitbar.
...
set(strainprog,'Position',[246 104.25 270 56.25]);
movie=figure;    %make a movie figure
for i = 1:time
    ...
    plot(Protocol(1:length(Strain),2),Strain,'k')
    axis([0 max(Protocol(:,2)) 0 1])
    M(i)=getframe(movie);
...
close(movie)
close(strainprog)
alert=waitbar(0,(sprintf('Please Wait\nSaving Movie as RunMovie.avi...\n(The bar will not progress, but the file is saving)')));
movie2avi(M,'RunMovie')
close(alert)
if beepio==1
    beep;
end
