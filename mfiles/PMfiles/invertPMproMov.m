...
%Movie Pseudocode
% 1 - Shift the PM space
% 2 - Run the Stress protocol
% 3 - Plot, capture, add to movie

function PMSpace=invertPMproMov(newPM,maxstress,ds,loops,con)
...
set(currentprog,'Position',[246 104.25 270 56.25]);
%make a special figure for the movie
movie=figure;
for k = 1:loops %Do it once fairly rough, and then do it again to smooth it.
    ...
    for j = 1:length(Protocol)                          %For every time point
        ...
        %make a movie frame and add it to our movie
        StrainNew2 = getStrain(newPM,Protocol,length(Protocol));
        plot(Protocol(:,2),Strain,'r'); %The goal stress-strain curve
        hold on
        plot(Protocol(:,2),StrainNew2,'b'); %The current stress-strain curve
        hold off
        axis([0 maxstress 0 1]);
        M((k-1)*length(Protocol)+j)=getframe(movie);
...
close(movie)
close(currentprog)
alert=waitbar(0,(sprintf('Please Wait\nSaving Movie as InvertMovie.avi...')));
movie2avi(M,'InvertMovie')
close(alert)
